...
# Controlador raiz: media entre modelo, view, HUD (com controles de autoplay
# integrados) e features isoladas em core_extensions/. Conforme
# docs/ARCHITECTURE.md: nao modifica logica do core, apenas orquestra
# e injeta dependencias.
...
func _on_dijkstra_pressed() -> void:
	...
	if is_instance_valid(_lbl_dijkstra_resultado):
		if dest < 0 or caminho.is_empty():
			_lbl_dijkstra_resultado.text = "Dijkstra: origem %d sem alcancaveis alem dela" % origem
		else:
			var partes: Array = []
			for no in caminho:
